...
class XLMModel(XLMPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.use_lang_emb:
            # self.lang_embeddings = nn.Embedding(self.n_langs, self.dim)
            self.lang_embeddings = LangFunction(self.n_langs, self.dim)
    ...
    def from_pretrained(self, model_path, config):
        ...
        if 'model' in save_model.keys():
            ...
            for name in save_model['model']:
                if "pred_layer" not in name:
                    # pretrained_dict['transformer.'+name[7:]]=save_model['model'][name]
                    pretrained_dict[name[7:]]=save_model['model'][name]
        else:
            for name in save_model:
                # pretrained_dict['transformer.'+name[7:]]=save_model[name]
                pretrained_dict[+name[7:]]=save_model['model'][name]
...
class XLMForSequenceClassification1(XLMPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def save_pretrained(self, output_dir):
        path = os.path.join(output_dir, 'ckpt-best.pth')
        ...
        for name in ['transformer', 'sequence_summary']:
...
class XLMForQuestionAnswering1(XLMPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class LangFunction(nn.Module):
    def __init__(self, n_langs=15, embed_dim=1024):
        super().__init__()

        self.n_langs = n_langs
        self.embed_dim = embed_dim
        self.lang_weight = torch.nn.Parameter(torch.FloatTensor(n_langs, embed_dim, embed_dim))
        self.lang_bias = torch.nn.Parameter(torch.FloatTensor(n_langs, embed_dim))

        self.reset_parameters()

    def reset_parameters(self) -> None:
        torch.nn.init.kaiming_uniform_(self.lang_weight, a=math.sqrt(5))
        if self.lang_bias is not None:
            fan_in, _ = torch.nn.init._calculate_fan_in_and_fan_out(self.lang_weight[0])
            bound = 1 / math.sqrt(fan_in)
            torch.nn.init.uniform_(self.lang_bias, -bound, bound)

    def forward(self, tensor, lang_tag=None, weight_transpose=False):
        bs, t_len, embed_dim = tensor.size()
        assert embed_dim == self.embed_dim

        x = tensor.contiguous().view(bs * t_len, -1).unsqueeze(-2).permute(1, 0, 2)
        if weight_transpose:
            x_h = torch.bmm(x.repeat(self.n_langs, 1, 1), torch.transpose(self.lang_weight, 1, 2)).permute(1, 0, 2)
            x = x_h + torch.bmm(x.repeat(self.n_langs, 1, 1), self.lang_bias.unsqueeze(-1)).permute(1, 0, 2)

            # x = torch.bmm(x.repeat(self.n_langs, 1, 1), torch.transpose(self.lang_weight, 1, 2)).permute(1, 0, 2)
            # x = x + torch.bmm(x.permute(1, 0, 2), self.lang_bias.unsqueeze(-1)).permute(1, 0, 2)
        else:
            x = torch.bmm(x.repeat(self.n_langs, 1, 1), self.lang_weight).permute(1, 0, 2) + self.lang_bias
            # x = torch.bmm(x.repeat(self.n_langs, 1, 1), self.lang_weight).permute(1, 0, 2)
        x = x.contiguous().view(bs, t_len, -1, embed_dim)

        if lang_tag is None:
            lang_tag = torch.ones(bs).to(x.device)
        else:
            lang_tag = lang_tag.float().mean(dim=1)
        lang_arange = torch.arange(0, self.n_langs).to(x.device)
        lang_mask = (lang_tag.unsqueeze(-1).to(lang_arange.dtype) == lang_arange).unsqueeze(1).unsqueeze(-1)
        output = x.masked_select(lang_mask).view(bs, -1, embed_dim)
        return output